...
#' 
#' @return The parameters 'g0', 'e', 'r.AM', 'nu' and 'N' are stored in an attribute named 'parameters'.
#'
#' @examples 
#' pop.evolution(1, 1, 0.6, 0.2, Inf, 10)
#'
#' @export
pop.evolution <- function(g0, e, r.AM, nu, N, nb.gen) {
  L <- pop(g0, e, r.AM, nu, N)
  R <- as.data.frame(L)
  for(i in 1:nb.gen) {
    L <- next.gen(L)
    R <- rbind(R, L)
  ...
  attr(R, "parameters") <- c(g0 = g0, e = e, r.AM = r.AM, nu = nu, N = N)
